...
class CombineDBs(data.Dataset):
    def __init__(self, dataloaders, excluded=None):
        ...
        for dl in dataloaders:
            for elem in dl.list_sample:
                if elem not in self.list_sample:
                    self.list_sample.append(elem)

        # Exclude
        if excluded:
            for dl in excluded:
                for elem in dl.list_sample:
                    if elem in self.list_sample:
                        self.list_sample.remove(elem)
        ...
        for ii, dl in enumerate(dataloaders):
            for jj, curr_im_id in enumerate(dl.list_sample):
                if (curr_im_id in self.list_sample) and (curr_im_id not in new_im_ids):
                    num_images += 1
                    new_im_ids.append(curr_im_id)
                    self.cat_list.append({'db_ii': ii, 'cat_ii': jj})

        self.list_sample = new_im_ids

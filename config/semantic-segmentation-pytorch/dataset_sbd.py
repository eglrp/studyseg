...
class TrainDataset(torchdata.Dataset):
    def __init__(self , opt, max_sample=-1, batch_per_gpu=1):
        ...
        self.list_sample = [x for x in open(opt.root_dataset + 'benchmark_RELEASE/dataset/train.txt', 'r').read().splitlines()]
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            image_path = os.path.join(self.root_dataset, 'benchmark_RELEASE/dataset/img' ,self.list_sample[self.cur_idx]+'.jpg' )#this_record['fpath_img'])
            segm_path = os.path.join(self.root_dataset, 'benchmark_RELEASE/dataset/cls' , self.list_sample[self.cur_idx]+'.mat')#this_record['fpath_segm'])
            self.cur_idx +=1
            if self.cur_idx >= self.num_sample:
                self.cur_idx = 0
                break
            ...
            segm = scipy.io.loadmat(segm_path)["GTcls"][0]['Segmentation'][0]
            assert(img.ndim == 3)
            assert(segm.ndim == 2)
